...
def calculate_expected_price_per_sqft_single(city, condition, medians_df):
    """
    Calculate expected price per sqft for a single property based on city and condition.
    Uses loaded city-condition medians from pickle file.
    """
    ...
    if not result.empty:
        ...
        # If combination not found, return a default value or raise an error
        st.warning(f"No data found for city {city} and condition {condition}")
